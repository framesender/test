			...
			else
			{
				echo ('<div style="margin: 10px 0 10px 0;">
						<form method="POST" action="'.$url.'">
							...
							<tfoot>	
							<tr>
								<td colspan="5" align="center"><h3>Извините, нет картинок для отображения!</h3></td>
				...
				else if (isset($_POST["add_data_form"]))
				{
					...
					if ($true == true)
					{
						...
						for($i=0; $i<$col; $i++)
						{
							...
							$name_b = $code.'photo_'.$max_gall_id[0].'.'.$ext_b;
							$name_s = $code.'thumb_photo_'.$max_gall_id[0].'.'.$ext_b;
							...
							if(in_array($ext_b, $extperms))
							{
								...
								if ($size1[0] > $size1[1]) img_resize('../images/galleria/'.$name_b, '../images/galleria/'.$name_s, 106, 73);
								else if ($size1[0] < $size1[1]) img_resize('../images/galleria/'.$name_b, '../images/galleria/'.$name_s, 73, 106);
