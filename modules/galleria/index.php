...
//                                    } else echo '<div class="sms_error">Эскиз не добавлен!</div>';
                                MyResize($destination_b, "../images/galleria/".$name_s, 120);
						...
						echo ('<form enctype="multipart/form-data" method="POST" action="'.$url.'" id="data">
								<table border="0">
									...
									<tr><td colspan="2" align="left">картинка (для просмотра)<br /><input type="file" name="image_b" value="" size="38" /><br />
                                            ...
                                            <br>Размер: <input type="text" name="sizex" size="2" value = "400">
					...
					if ($_POST['image_b_old'])
					{
						...
						if (!empty($_FILES["image_b"]["name"]))
						{
							...
							if(in_array($ext_b, $extperms))
							{
                                ...
                                MyResize($destination_b, $file_s, 120);
        /*                      
								$image_b = "'".$name_b."'";
								$size = getimagesize('../images/galleria/'.$name_b);
								if ($size[0] > $size[1]) img_resize('../images/galleria/'.$name_b, '../images/galleria/'.$name_b, 640, 480);
								else if ($size[0] < $size[1]) img_resize('../images/galleria/'.$name_b, '../images/galleria/'.$name_b, 480, 640);
                                ...
                                //$image->image_to_resize = $destination_b;
                                //$image->ratio = true;
                                //$image->new_image_name = substr($name_b, 0, -4);
                                //$image->save_folder = '../images/galleria/';
                                //$process = $image->resize();
                                //if($process['result'] && $image->save_folder)
                                //    {
                                //    echo '<div class="sms_succses">Фото ('.$process['new_file_path'].') сохранено.</div>';
                                //    } else echo '<div class="sms_error">Фото не добавлено!</div>';
                                //$image->new_image_name = substr($name_s, 0, -4);
                                //$image->new_width = 120;
                                //$image->new_height = 120;
                                //$process = $image->resize();
                                //if($process['result'] && $image->save_folder)
						...
						else { $image_b = "'".$_POST['image_b_old']."'"; $image_s = "'".$_POST['image_s_old']."'"; }
...
function add() {
  ...
  id++;
  $("div[id=100]").append('<div id="div-'+id+'"><table style="border: 1px solid #ffffff; padding: 10px; margin-top: 10px;"><tr><td>описание<br /><textarea name="description[]" cols="38" rows="7"></textarea></td><td>картинка (для просмотра)*<br /><input type="file" name="image_b[]" value="" size="38" /><br />сортировка<br /><input type="text" name="gallery_sort[]" value="" size="10" /></td></tr><tr><td>размер: <input type="text" name="sizex[]" size="2" value = "400"></td></tr></table><span id="cell"><a href="javascript:{}" onclick="removeInput(\'' + id + '\')">Удалить</a></span></div>');
